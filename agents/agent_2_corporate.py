...
def agent_2_corporate_scraper_persona_finder(state: AgentState) -> dict:
	"""
	Agent 2: Uses the law context plus the selected region/sector to locate the
	matching company report, read it from the local vault, and extract the CSO.
	"""
	print(f"--- [Agent 2] Searching local company report for {state.get('target_region')} / {state.get('target_sector')} ---")
	...
	if not selected:
		return {
			**state,
			"raw_corporate_report": (
				f"No local company report mapped for region='{state.get('target_region')}' "
